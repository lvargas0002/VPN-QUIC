...
int main() {
    ...
    ptls_aead_context_t *aead = ptls_aead_new(suite->aead, suite->hash, 1, key, "key-label");
    if(aead == NULL){
    ...
    size_t encrypted_len = ptls_aead_encrypt(aead, encrypted, plain, plain_len, 0, NULL, 0);
    if(encrypted_len == 0){
